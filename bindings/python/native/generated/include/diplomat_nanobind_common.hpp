...
namespace nanobind::detail
{
    ...
    struct type_caster<_native::diplomat::result<T, E>>
    {
        ...
        static handle from_cpp(_native::diplomat::result<T, E> value, rv_policy p, cleanup_list *cl) noexcept
        {
            if (value.is_ok()) {
                if constexpr (std::is_same_v<U, std::monostate>) {
                    return none().release();
        ...
        operator Value() { 
            if (is_ok) {
                return _native::diplomat::Ok<T>(forward_like_<U>(ok_val.value()));
            } else {
                return _native::diplomat::Err<E>(forward_like_<V>(err_val.value()));
            }
        }
    ...
    class type_caster<_native::diplomat::span<T, E>> {
    ...
    public:
        using Value = _native::diplomat::span<T, E>;
        Value value = _native::diplomat::span<T, E>();
        ...
        bool from_python(handle src, uint8_t flags, cleanup_list* cleanup) noexcept {
            ...
            if constexpr (is_ndarray_scalar_v<T>) {
                ...
                if (caster.from_python(src, local_flags, cleanup)) {
                    ...
                    value = _native::diplomat::span<T, E>(caster.value.data(), caster.value.shape(0));
                    return true;
                }
            }
            ...
            if (nb::inst_check(src)) {
                ...
                if (inf == typeid(U)) {
                    ...
                    value = _native::diplomat::span<T, E>(reinterpret_cast<T*>(bound_vec->data()), bound_vec->size());
                    return true;
                }
            }
            ...
            ListCaster caster;
            if (caster.from_python(src, local_flags, cleanup)) {
                value = _native::diplomat::span<T, E>(reinterpret_cast<T*>(caster.value.data()), caster.value.size());
        ...
        static handle from_cpp(_native::diplomat::span<T, E> src, rv_policy policy, cleanup_list* cleanup) {
    ...
    template <>
    struct type_caster<_native::diplomat::string_view_for_slice> {
        NB_TYPE_CASTER(_native::diplomat::string_view_for_slice, const_name("str"))
        ...
        bool from_python(handle src, uint8_t, cleanup_list *) noexcept {
            ...
            value = _native::diplomat::string_view_for_slice(str, (size_t) size);
            return true;
        }

        static handle from_cpp(_native::diplomat::string_view_for_slice value, rv_policy, cleanup_list *) noexcept {
...
struct get_diplomat_result<_native::diplomat::result<T, E>> : std::true_type {
    typedef _native::diplomat::Ok<T> ok;
    typedef _native::diplomat::Err<E> err;

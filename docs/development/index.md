...
Headers are generated by cbindgen and written to `fatoora-ffi/include/` during the build. Do not edit them manually. `fatoora_ffi.h` uses the `fatoora_` prefix; `fatoora.h` provides alias names without the prefix when compiled with `-DFATOORA_FFI_NO_PREFIX`. Module headers are also available under `fatoora/` (e.g., `fatoora/config.h`, `fatoora/invoice.h`).

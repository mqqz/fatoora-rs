...
- Signed invoice metadata getters (signature/public key/signed props hashes/signing time as a `YYYY-MM-DDTHH:MM:SS` UTC string).
- Module headers are available under `fatoora/` for grouping (e.g., `fatoora/config.h`, `fatoora/invoice.h`).
- Invoice builder timestamps use ZATCA ISO UTC strings (`YYYY-MM-DDTHH:MM:SSZ`).

...
def ksa_arithmetic_cases(base):
    ...
    for value, count in [("345", 0), ("345.005", 0), ("345.015", 1), ("345.01", 1)]:
        case(
            "line-inclusive-" + value,
            lambda r, v=value: setattr(
                r.find("cac:InvoiceLine/cac:TaxTotal/cbc:RoundingAmount", NS), "text", v
            ),
            "BR-KSA-51",
            count,
        )

    def foreign(root, value):
        root.find("cbc:DocumentCurrencyCode", NS).text = "USD"
        for element in root.iter():
            if "currencyID" in element.attrib:
                element.set("currencyID", "USD")
        bare = root.find("cac:TaxTotal/cbc:TaxAmount", NS)
        bare.set("currencyID", "SAR")
        bare.text = value

    for value, count in [("118.88", 1), ("118.8801", 1), ("118.89", 0), ("0", 0)]:
        case(
            "foreign-tax-" + value, lambda r, v=value: foreign(r, v), "BR-KSA-97", count
        )

    def adjustment(root, amount, base_amount, percent):
        adj = root.find("cac:AllowanceCharge", NS)
        node = adj.find("cbc:Amount", NS)
        node.text = amount
        factor = ET.Element(f"{{{NS['cbc']}}}MultiplierFactorNumeric")
        factor.text = percent
        adj.insert(list(adj).index(node), factor)
        base_node = ET.Element(f"{{{NS['cbc']}}}BaseAmount", {"currencyID": "SAR"})
        base_node.text = base_amount
        adj.insert(list(adj).index(node) + 1, base_node)

    for amount, base_amount, percent, count in [
        ("20", "100", "20", 0),
        ("20.01", "100", "20", 1),
        ("1.01", "1.005", "100", 1),
        ("1.00", "1.005", "100", 0),
        ...
        case(
            "adjustment-" + base_amount + "-" + amount,
            lambda r, a=amount, b=base_amount, p=percent: adjustment(r, a, b, p),
            "BR-KSA-EN16931-03",
            count,
        )

    def line_amount(root, quantity, price, base_quantity, amount):
        line = root.find("cac:InvoiceLine", NS)
        line.find("cbc:InvoicedQuantity", NS).text = quantity
        line.find("cbc:LineExtensionAmount", NS).text = amount
        node = line.find("cac:Price", NS)
        node.find("cbc:PriceAmount", NS).text = price
        if base_quantity is not None:
            ET.SubElement(
                node, f"{{{NS['cbc']}}}BaseQuantity", {"unitCode": "HUR"}
            ).text = base_quantity
    ...
    for amount, count in [
        ...
        case(
            ...
            lambda r, a=amount: line_amount(r, "2", "150", None, a),
            "BR-KSA-EN16931-11",
            count,
        )
    # Literal expectations from independent SDK probes; the SDK's decimal
    # division uses normalized operand scales and 18 places with ties to zero.
    for name, quantity, price, divisor, amount, count in [
        ("third", "3", "1", "3", "1.00", 0),
        ("scaled-third", "100000000000000000000", "1", "3", "33333333333333333300", 0),
        (
            "exact-rational-differs",
        ...
        (
            ...
            "3",
            "524288",
            "572204589843800000000",
            1,
        ),
    ]:
        case(
            "division-" + name,
            lambda r, q=quantity, p=price, d=divisor, a=amount: line_amount(
                r, q, p, d, a
            ),
            "BR-KSA-EN16931-11",
            count,
    ...
    case(
        "division-by-zero",
        lambda r: line_amount(r, "2", "150", "0", "300"),
        "SaxonApiException",
        1,
        "error",
    )
...
def ksa_arithmetic_guard_cases(base):
    cases = []
    for name, missing, invalid in [
        (
            "missing-net-invalid-quantity",
            "cbc:LineExtensionAmount",
            "cbc:InvoicedQuantity",
        ),
        (
            "missing-quantity-invalid-price",
            "cbc:InvoicedQuantity",
            "cac:Price/cbc:PriceAmount",
        ),
        (
            "missing-price-invalid-base",
            "cac:Price/cbc:PriceAmount",
            "cac:Price/cbc:BaseQuantity",
        ),
        ("missing-net-zero-base", "cbc:LineExtensionAmount", "zero-base"),
        ("missing-quantity-zero-base", "cbc:InvoicedQuantity", "zero-base"),
    ]:
        root = ET.fromstring(base)
        line = root.find("cac:InvoiceLine", NS)
        parent = line.find(missing.rsplit("/", 1)[0], NS) if "/" in missing else line
        parent.remove(line.find(missing, NS))
        if invalid in ["cac:Price/cbc:BaseQuantity", "zero-base"]:
            node = ET.SubElement(
                line.find("cac:Price", NS),
                f"{{{NS['cbc']}}}BaseQuantity",
                {"unitCode": "HUR"},
            )
        else:
            node = line.find(invalid, NS)
        node.text = "0" if invalid == "zero-base" else "1E0"
        cases.append(
            {
                "id": name,
                "xml": ET.tostring(root, encoding="unicode"),
                "expected_xsd": "passed" if name == "missing-quantity-zero-base" else "failed",
                "targets": [
                    {
                        "source": "ksa",
    ...
    root = ET.fromstring(base)
    line = root.find("cac:InvoiceLine", NS)
    adj = ET.Element(f"{{{NS['cac']}}}AllowanceCharge")
    ET.SubElement(adj, f"{{{NS['cbc']}}}ChargeIndicator").text = "true"
    for value in ["1", "2"]:
        ET.SubElement(adj, f"{{{NS['cbc']}}}Amount", {"currencyID": "SAR"}).text = value
    line.insert(list(line).index(line.find("cac:TaxTotal", NS)), adj)
    cases.append(
        {
            "id": "duplicate-adjustment-amounts",
            "xml": ET.tostring(root, encoding="unicode"),
            "expected_xsd": "failed",
            "targets": [
                {

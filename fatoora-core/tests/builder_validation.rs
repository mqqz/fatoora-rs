...
fn credit_note_invalid_line_item_reports_issues() {
    ...
    assert!(validation.issues().iter().any(|issue| {
        issue.field() == InvoiceField::LineItemQuantity && issue.kind() == ValidationKind::OutOfRange
    }));
    assert!(validation.issues().iter().any(|issue| {
        issue.field() == InvoiceField::LineItemUnitPrice && issue.kind() == ValidationKind::OutOfRange
    }));
    assert!(validation.issues().iter().any(|issue| {
        issue.field() == InvoiceField::LineItemTotalAmount && issue.kind() == ValidationKind::OutOfRange
    }));
    assert!(validation.issues().iter().any(|issue| {
        issue.field() == InvoiceField::LineItemVatAmount && issue.kind() == ValidationKind::OutOfRange
    }));
}
